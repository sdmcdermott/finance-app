...
func handler(ctx context.Context, req events.APIGatewayV2HTTPRequest) (response, error) {
	...
	ruleID := req.PathParameters["ruleId"]
	if ruleID == "" {
		return errorResponse(http.StatusBadRequest, "ruleId path parameter is required"), nil
	}
	...
	if err := dbClient.DeleteRule(ctx, plaidclient.UserID(), ruleID); err != nil {
		return errorResponse(http.StatusInternalServerError, err.Error()), nil
	}

	body, _ := json.Marshal(map[string]string{"deleted": ruleID})
